    ...
    <div v-if="currentSong" class="player-controls flex flex-col items-center gap-6 w-full max-w-md">
        ...
        <div class="relative group">
            <Tilted :width="isMobile ? '30vh' : '38vh'" :height="isMobile ? '30vh' : '38vh'" :rotateAmplitude="15" cardClass="rounded-2xl shadow-2xl" :scale="true"
                ...
                <div class="w-full h-full relative z-10 overflow-hidden bg-black/5 rounded-2xl">
                    <Transition name="cover-fade" mode="out-in">
                        <img :key="currentSong.pic" :src="currentSong.pic" :alt="currentSong.title"
                    ...
                    <!-- Loading Placeholder -->
                    <div
                        class="absolute inset-0 flex items-center justify-center -z-10 bg-linear-to-br from-gray-100 to-gray-200 animate-pulse">
            ...
            <Transition name="glow-fade">
                <div :key="currentSong.pic"
                    class="absolute inset-0 -z-10 bg-red-500/10 blur-3xl rounded-full scale-110"></div>
            </Transition>
        </div>
